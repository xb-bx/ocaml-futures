...
let send_file socket path =
    ...
    | _ -> 
        send_404 socket
let prepare_dir_message base files = 
    let base = match String.ends_with ~suffix:"/" base with
    | true -> base 
    | false -> String.cat base "/" in
    ...
    in prep "" files
let send_dir socket path =
    let files = Sys.readdir path |> Array.to_list in
    let html = prepare_dir_message path files in
...
let client socket = 
        ...
        if nread = 0 then 
            ...
            match req with 
            | Some(path) -> 
                let path = (url_decode path) in
                let^ _ = 
                    (match check_path path with 
                        | Some(Dir) -> send_dir socket path
...
let listenFut port = 
    ...
    let^ ctx = get_ctx in
    let^ _ = futureForeverLoop (fun _-> let^ c = uring_accept socket clientaddr in enqueue_future ctx (client (Obj.magic c)); finished ()) in
    finished ()
let () = 
    let port = match Array.to_list Sys.argv with 
    | _ :: port :: _ when int_of_string_opt port |> Option.is_some -> int_of_string port 
    | _ -> 6969 in
    let q = Cqueue.of_list [listenFut port] in
